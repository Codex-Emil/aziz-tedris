      ...
      if (isPassiveOrFrozen) {
        ...
        if (p.packageType === "Aylıq") {
          ...
          if (p.paymentStatus === "Ödənilməyib") {
            ...
            let statusText = p.paymentStatus === "Qismən ödənilib" ? `Qismən (${p.paidAmount}/${p.fee} AZN)` : "Aktiv";
            const badgeClass = p.paymentStatus === "Qismən ödənilib" ? "badge-partial" : "badge-paid";
            if (remaining <= 0) {
              ...
              statusHtml = `<span class="badge ${badgeClass}">${statusText} (${remaining} seans qalıb)</span>`;
    ...
    const teacherRowsHtml = teachers.map(t => {
      ...
      return `
        <tr>
          <td>
            <strong>${t.name}</strong>
            <div style="font-size: 0.75rem; color: #64748b; font-weight: normal; margin-top: 0.15rem;">
              • ${coursesStr}
            </div>
          </td>
          <td>${stdCount} tələbə</td>
          <td>${revenue} AZN</td>
          <td>${sharePercent}%</td>
          <td>${teacherShare.toFixed(1)} AZN</td>
          <td>${paid} AZN</td>
          <td style="color: ${due > 0 ? '#b45309' : (due < 0 ? '#b91c1c' : '#047857')}; font-weight: bold;">${due.toFixed(1)} AZN</td>
        </tr>
      `;
    }).join("");
